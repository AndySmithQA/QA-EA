...
const SellerProperty = (props) => {
    ...
    useEffect(() => {
        ...
        fetch("http://localhost:8080/seller/" + sellerId)
        // get the JSON content from the response
        .then((response) => {
            if (!response.ok) {
                alert("An error has occurred.  Unable to get Seller information");
                throw response.status;
            } else {
                return response.json();
            }
    ...
    return (
        <>
            <div className="pageHeader"><i className="bi bi-person-square"/>&nbsp;Manage Seller Properties
                for <span
                    className={"text-info"}>REF:{sellerId} - {seller.firstName} {seller.surname} - {seller.address}</span>
            </div>
            ...
            <ul>
                {
                    properties.filter(property => Number(property.seller.id) === Number(sellerId)).length === 0 && !loadingProperties ?
                        <li>
                            <div className="message alert alert-info" role="alert">
                        ...
                        :
                        properties.filter(property => Number(property.seller.id) === Number(sellerId)).map(property => (
                            <li key={property.id}>
